...
def create_table(realec_data, folder='.'):
    """
    ...
    features = None
    for ind, (name, text) in tqdm(enumerate(realec_data.items()), total=len(realec_data)):
        text = re.sub(r'<.*?@\n', '', text)
        features = main(text)
        features.keys()
        data = {}
    ...
    path = os.path.join(folder, "dataset_data")
    print("Result written in: " + path)
...
if __name__ == '__main__':
    ...
    realec_data = {}
    folder = "data_for_inspector"
    print("Folder for analysis: \n" + folder + '/*.txt')
    for filepath in glob.glob(folder + '/*.txt', recursive=True):
        # file = filepath.split("/")[-1]
        # print(filepath)
        if filepath[-4:] == ".txt":  
    #         print("Sdsdsd")
            with open(filepath, "r") as f:
                if filepath in realec_data:
                    print(filepath)
                realec_data[filepath] = f.read() #.strip().replace("\n", "").replace("\t", "")
    print(f"Num of texts: {len(realec_data)}")
    create_table(realec_data)
